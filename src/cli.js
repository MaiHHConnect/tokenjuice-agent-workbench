 ...
 *   node src/cli.js create <标题> [--skills=frontend,backend]
 *   node src/cli.js claim <taskId> <agentId>
 *   node src/cli.js status <taskId> <newStatus>
...
async function createTask(title, description, skills = []) {
...
async function reportBug(taskId, bugReport) {
  return request(`/api/tasks/${taskId}/bug`, {
    method: 'POST',
    body: JSON.stringify({ bugReport })
  })
}

async function registerAgent(name, role, capabilities = []) {
...
async function main() {
  ...
  switch (command) {
    ...
    case 'create': {
      ...
      if (!title) {
        console.error('用法: node src/cli.js create <标题> [--skills=frontend,backend]')
        process.exit(1)
      }
    ...
    case 'claim': {
      ...
      if (!taskId || !agentId) {
        console.error('用法: node src/cli.js claim <taskId> <agentId>')
        process.exit(1)
      }
    ...
    case 'status': {
      ...
      if (!taskId || !newStatus) {
        console.error('用法: node src/cli.js status <taskId> <newStatus>')
    ...
    case 'bug': {
      ...
      if (!taskId || !bugReport) {
        console.error('用法: node src/cli.js bug <taskId> <bug描述>')
        process.exit(1)
      }
    ...
    case 'register': {
      ...
      if (!name || !role) {
        console.error('用法: node src/cli.js register <名称> <角色> [--skills=frontend,backend]')
